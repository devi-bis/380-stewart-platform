...
const int SERVOMIN = 125; // minimum 'tick' value between 0..4095 -> used in setPWM(pin, on-tick, off-tick)
const int SERVOMAX = 525; // maximum 'tick' value between 0..4095 -> used in setPWM(pin, on-tick, off-tick)
const int SERVOMID = floor((SERVOMAX + SERVOMIN) / 2); // mid 'tick' value
const int SERVOCHG = 50; // 'change' value
...
void setup()
{
  ...
  Serial.println("\nServo Motor - Stewart Platform Actuation");
  Serial.println("Enter command: [# from 1 to 6], [q,e,r,t,y], or [<, m, >, +, -]");
